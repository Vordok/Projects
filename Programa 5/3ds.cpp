...
void CLoad3DS::Render3DSFile(t3DModel *pModel, CTga *Textura, int tipo)
{
	...
	datos[0] = pModel->numOfObjects;
	datos[3] = pModel->numOfMaterials;
	...
	for(int i = 0; i < pModel->numOfObjects; i++)
	{
		...
		pol += pObject->numOfFaces;
		vert += pObject->numOfVerts;		
	}
	datos[1] = pol;
	datos[2] = vert;
}

void CLoad3DS::Render3DSContour(t3DModel *pModel)
{
	// Since we know how many objects our model has, go through each of them.
	for(int i = 0; i < pModel->numOfObjects; i++)
	{
...
// This returns the point product between 2 vectors
