...
void CShader::InitShaders(string strVertex, string strFragment)
{
	...
	if (logLength > 1)
	{
		...
		MessageBox(hWnd, szLog, "GLSL Error", MB_OK);
...
void CShader::sendUniform(GLuint location, int x)
{
	glUniform1iARB(location, x);
}

GLuint CShader::getUniformLocation(const string &name) const
{
	return glGetUniformLocationARB(handle_,name.c_str());
}

void CShader::EnvParameter(unsigned int index, float a, float b, float c, float d)
{
   if (target != 0 && id != 0)
   {
...
void CShader::BindVariable(string strVariable, const float* value, size_t size)
{
   if(m_hProgramObject)
   {
		unsigned int location = glGetUniformLocationARB(m_hProgramObject, strVariable.c_str()); /* This is EXPENSIVE
