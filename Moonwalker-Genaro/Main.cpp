...
//Variables para el tama�o del cubo del ataque de fuego de MJ
int l=4;	//largo
int h=4;	//alto
int w=10;	//ancho
CVector pf; //punto origen del fuego (posici�n de las manos de MJ)
//pf = CVector( 0.0f, 0.0f, 0.0f);
int MJAtaque=0; //0 no ataca, 1 si ataca
...
parametros enem3e;	 //Variable con la que tenemos acceso a la estructura de par�metros de ene3e
parametros enem3f;	 //Variable con la que tenemos acceso a la estructura de par�metros ene3f
parametros enem3g;	 //Variable con la que tenemos acceso a la estructura de par�metros de ene3g
parametros enem3h;	 //Variable con la que tenemos acceso a la estructura de par�metros ene3h
...
//nombre y ubicaci�n de modelo Enemigo1
...
//nombre y ubicaci�n de modelo MJ6 (MJ Robot)
...
//nombre y ubicaci�n de modelo Ene3a
...
//nombre y ubicaci�n de modelo ene3b
...
//nombre y ubicaci�n de modelo chango
...
int tipoAnim = 1; //Indicador del tipo de animaci�n
...
//Constantes de iluminaci�n y materiales
GLfloat LightPos[] = { 100.0f, 20.0f, 0.0f, 1.0f};		// Posici�n de la luz
...
//Variables auxiliares para animaci�n de texturas
float aText1;
float aText2;
...
//Posici�n de la luz para el espacio de objeto del modelo1
...
//Variables para el c�lculo de transformaciones inversas para las sombras
...
void InicializaParametrosdeControl()
{
	...
	player1.CamaraPosAlt = 10.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	player1.CamaraObjAlt = 6.4f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	player1.AngDir = 90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	player1.AngObj = 0.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
	...
	player1.PosicionObj = CVector( 220.0f, 4.0f, -45.0f); //Esta es la posici�n inicial del objeto en la escena
	player1.Direccion.x = cosf( player1.AngDir * PI / 180.0f); //Direcci�n inicial definida por el �ngulo inicial AngDir (x=cos(AngDir), y=0.0, z=sen(AngDir))
	player1.Direccion.y = 0.0f;
	player1.Direccion.z = sinf( player1.AngDir * PI / 180.0f);   
	player1.PosicionCam = CVector( player1.PosicionObj.x, player1.PosicionObj.y + player1.CamaraPosAlt, player1.PosicionObj.z +player1.DistanciaCam ); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	...
	miku.CamaraPosAlt = 10.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	miku.CamaraObjAlt = 6.4f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	miku.AngDir = 90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	miku.AngObj = 0.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	miku.PosicionObj = CVector( 35.0f, 7.7f, -90.0f); //Esta es la posici�n inicial del objeto en la escena
	miku.Direccion.x = cosf( miku.AngDir * PI / 180.0f); //Direcci�n inicial definida por el �ngulo inicial AngDir (x=cos(AngDir), y=0.0, z=sen(AngDir))
	miku.Direccion.y = 0.0f;
	miku.Direccion.z = sinf( miku.AngDir * PI / 180.0f);   
	miku.PosicionCam = CVector( miku.PosicionObj.x, miku.PosicionObj.y + miku.CamaraPosAlt, miku.PosicionObj.z +miku.DistanciaCam ); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	miku.ObjetivoCam = miku.PosicionObj;		//La c�mara ve siempre al objeto
	...
	enemigo8.CamaraPosAlt=0.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	enemigo8.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	enemigo8.AngDir=-PI;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	enemigo8.AngObj=PI;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	...
	enemigo8.PosicionCam=CVector(0.0f, enemigo8.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	enemigo8.ObjetivoCam=enemigo8.PosicionObj;		//La c�mara ve siempre al objeto
	...
	enem1.CamaraPosAlt=5.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	enem1.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	enem1.AngDir=90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	enem1.AngObj=0.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	enem1.PosicionObj=CVector(180.0f, 9.0f, -60.0f); //Esta es la posici�n inicial del objeto en la escena
	enem1.Direccion.x=(float)cos(player1.AngDir*PI/180.0f); //Direcci�n inicial definida por el �ngulo inicial AngDir (x=cos(AngDir), y=0.0, z=sen(AngDir))
	enem1.Direccion.y=0.0f;
	enem1.Direccion.z=(float)sin(player1.AngDir*PI/180.0f);   
	enem1.PosicionCam=CVector(0.0f, player1.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	enem1.ObjetivoCam=player1.PosicionObj;		//La c�mara ve siempre al objeto
	...
	enem1a.PosicionObj=CVector(35.0f, 7.7f, -60.0f); //Esta es la posici�n inicial del objeto en la escena
	...
	// Enem1b
	enem1b.visible=true;
	enem1b.VelocidadObj=0.2f;
	enem1b.DistanciaCam=10.0f;

	enem1b.CamaraPosAlt=5.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	enem1b.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	enem1b.AngDir=90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	enem1b.AngObj=0.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	enem1b.PosicionObj=CVector(-15.0f, 8.0f, -60.0f); //Esta es la posici�n inicial del objeto en la escena
	enem1b.Direccion.x=(float)cos(player1.AngDir*PI/180.0f); //Direcci�n inicial definida por el �ngulo inicial AngDir (x=cos(AngDir), y=0.0, z=sen(AngDir))
	enem1b.Direccion.y=0.0f;
	enem1b.Direccion.z=(float)sin(player1.AngDir*PI/180.0f);   
	enem1b.PosicionCam=CVector(0.0f, player1.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	enem1b.ObjetivoCam=player1.PosicionObj;		//La c�mara ve siempre al objeto
	enem1b.ObjetivoCam.y=player1.CamaraObjAlt;		//Para que no vea a los "pies" del objeto (personaje)
	...
	enem1b.CamaraObjAltE=0.0f;

       // Enem1c
	enem1c.visible=true;
	enem1c.VelocidadObj=0.2f;
	enem1c.DistanciaCam=10.0f;

	enem1c.CamaraPosAlt=5.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	enem1c.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	enem1c.AngDir=90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	enem1c.AngObj=0.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	enem1c.PosicionObj=CVector(-57.5f, 8.0f, 100.5f); //Esta es la posici�n inicial del objeto en la escena
	...
	enem1c.PosicionCam=CVector(0.0f, player1.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	enem1c.ObjetivoCam=player1.PosicionObj;		//La c�mara ve siempre al objeto
	enem1c.ObjetivoCam.y=player1.CamaraObjAlt;		//Para que no vea a los "pies" del objeto (personaje)
	...
	enem2.CamaraPosAlt=5.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	enem2.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	enem2.AngDir=90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	enem2.AngObj=0.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	enem2.PosicionObj=CVector(120.0f, 10.0f, -80.0f); //Esta es la posici�n inicial del objeto en la escena
	enem2.Direccion.x=(float)cos(player1.AngDir*PI/180.0f); //Direcci�n inicial definida por el �ngulo inicial AngDir (x=cos(AngDir), y=0.0, z=sen(AngDir))
	enem2.Direccion.y=0.0f;
	enem2.Direccion.z=(float)sin(player1.AngDir*PI/180.0f);   
	enem2.PosicionCam=CVector(0.0f, player1.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	enem2.ObjetivoCam=player1.PosicionObj;		//La c�mara ve siempre al objeto
	...
	MJ6.CamaraPosAlt=5.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	MJ6.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	MJ6.AngDir=90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	MJ6.AngObj=0.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	MJ6.PosicionObj=CVector(-15.0f, 8.5f, -65.0f); //Esta es la posici�n inicial del objeto en la escena
	MJ6.Direccion.x=(float)cos(player1.AngDir*PI/180.0f); //Direcci�n inicial definida por el �ngulo inicial AngDir (x=cos(AngDir), y=0.0, z=sen(AngDir))
	MJ6.Direccion.y=0.0f;
	MJ6.Direccion.z=(float)sin(player1.AngDir*PI/180.0f);   
	MJ6.PosicionCam=CVector(0.0f, player1.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	MJ6.ObjetivoCam=player1.PosicionObj;		//La c�mara ve siempre al objeto
	...
	enem3a.CamaraPosAlt=5.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	enem3a.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	enem3a.AngDir=90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	enem3a.AngObj=0.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	enem3a.PosicionObj=CVector(-15.0f, 8.0f, -5.0f); //Esta es la posici�n inicial del objeto en la escena
	enem3a.Direccion.x=(float)cos(player1.AngDir*PI/180.0f); //Direcci�n inicial definida por el �ngulo inicial AngDir (x=cos(AngDir), y=0.0, z=sen(AngDir))
	enem3a.Direccion.y=0.0f;
	enem3a.Direccion.z=(float)sin(player1.AngDir*PI/180.0f);   
	enem3a.PosicionCam=CVector(0.0f, player1.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	enem3a.ObjetivoCam=player1.PosicionObj;		//La c�mara ve siempre al objeto
	...
	enem3b.CamaraPosAlt=5.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	enem3b.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	enem3b.AngDir=90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	enem3b.AngObj=270.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	enem3b.PosicionObj=CVector(-19.0f, 8.0f, -10.0f); //Esta es la posici�n inicial del objeto en la escena
	enem3b.Direccion.x=(float)cos(player1.AngDir*PI/180.0f); //Direcci�n inicial definida por el �ngulo inicial AngDir (x=cos(AngDir), y=0.0, z=sen(AngDir))
	enem3b.Direccion.y=0.0f;
	enem3b.Direccion.z=(float)sin(player1.AngDir*PI/180.0f);   
	enem3b.PosicionCam=CVector(0.0f, player1.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	enem3b.ObjetivoCam=player1.PosicionObj;		//La c�mara ve siempre al objeto
	...
	enem3c.PosicionObj=CVector(160.0f, 8.0f, -80.0f); //Esta es la posici�n inicial del objeto en la escena
	...
	enem3d.PosicionObj=CVector(200.0f, 8.0f, -40.0f); //Esta es la posici�n inicial del objeto en la escena
	...
	// enem3e
	enem3e.visible=true;
	enem3e.VelocidadObj=0.2f;
	enem3e.DistanciaCam=10.0f;

	enem3e.CamaraPosAlt=5.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	enem3e.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	enem3e.AngDir=90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	enem3e.AngObj=0.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	enem3e.PosicionObj=CVector(50.0f, 7.7f, -60.0f); //Esta es la posici�n inicial del objeto en la escena
	enem3e.Direccion.x=(float)cos(player1.AngDir*PI/180.0f); //Direcci�n inicial definida por el �ngulo inicial AngDir (x=cos(AngDir), y=0.0, z=sen(AngDir))
	enem3e.Direccion.y=0.0f;
	enem3e.Direccion.z=(float)sin(player1.AngDir*PI/180.0f);   
	enem3e.PosicionCam=CVector(0.0f, player1.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	enem3e.ObjetivoCam=player1.PosicionObj;		//La c�mara ve siempre al objeto
	enem3e.ObjetivoCam.y=player1.CamaraObjAlt;		//Para que no vea a los "pies" del objeto (personaje)
	...
	enem3e.CamaraObjAltE=0.0f;

	// enem3f
	enem3f.visible=true;
	enem3f.VelocidadObj=0.2f;
	enem3f.DistanciaCam=10.0f;

	enem3f.CamaraPosAlt=5.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	enem3f.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	enem3f.AngDir=90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	enem3f.AngObj=270.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	enem3f.PosicionObj=CVector(10.0f, 7.7f, -60.0f); //Esta es la posici�n inicial del objeto en la escena
	enem3f.Direccion.x=(float)cos(player1.AngDir*PI/180.0f); //Direcci�n inicial definida por el �ngulo inicial AngDir (x=cos(AngDir), y=0.0, z=sen(AngDir))
	enem3f.Direccion.y=0.0f;
	enem3f.Direccion.z=(float)sin(player1.AngDir*PI/180.0f);   
	enem3f.PosicionCam=CVector(0.0f, player1.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	enem3f.ObjetivoCam=player1.PosicionObj;		//La c�mara ve siempre al objeto
	enem3f.ObjetivoCam.y=player1.CamaraObjAlt;		//Para que no vea a los "pies" del objeto (personaje)
	...
	enem3f.CamaraObjAltE=0.0f;

	
	 // enem3g
	enem3g.visible=true;
	enem3g.VelocidadObj=0.2f;
	enem3g.DistanciaCam=10.0f;

	enem3g.CamaraPosAlt=5.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	enem3g.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	enem3g.AngDir=90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	enem3g.AngObj=0.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	enem3g.PosicionObj=CVector(-57.5f, 8.0f,  50.4f); //Esta es la posici�n inicial del objeto en la escena
	...
	enem3g.PosicionCam=CVector(0.0f, player1.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	enem3g.ObjetivoCam=player1.PosicionObj;		//La c�mara ve siempre al objeto
	enem3g.ObjetivoCam.y=player1.CamaraObjAlt;		//Para que no vea a los "pies" del objeto (personaje)
	...
	enem3g.CamaraObjAltE=0.0f;

	// enem3h
	enem3h.visible=true;
	enem3h.VelocidadObj=0.2f;
	enem3h.DistanciaCam=10.0f;

	enem3h.CamaraPosAlt=5.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	enem3h.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	enem3h.AngDir=90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	enem3h.AngObj=270.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	enem3h.PosicionObj=CVector(-57.5f, 8.0f,  150.4f); //Esta es la posici�n inicial del objeto en la escena
	...
	enem3h.PosicionCam=CVector(0.0f, player1.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	enem3h.ObjetivoCam=player1.PosicionObj;		//La c�mara ve siempre al objeto
	enem3h.ObjetivoCam.y=player1.CamaraObjAlt;		//Para que no vea a los "pies" del objeto (personaje)
	...
	chang.CamaraPosAlt=5.0f;	//Posici�n en y de la c�mara (altura a la que se situa la c�mara)
	chang.CamaraObjAlt=4.0f;	//Posici�n en y del objetivo de la c�mara (altura a la que ve la c�mara)
	chang.AngDir=90.0f;		//Este �ngulo inicial hace que la direcci�n inicial sea paralela al eje Z y con sentido negativo
	chang.AngObj=0.0f;		//Este valor se elige dependiendo de la orientaci�n con la que aparece el modelo en la escena al dibujarlo
								//sin aplicarle ninguna transformaci�n (hacia adonde est� volteando). Se elige un �ngulo tal que al aplicarle
								//una rotaci�n inicial con respecto al eje Y est� viendo hacia la misma direcci�n que la definida por AngDir
	
	chang.PosicionObj=CVector(-30.0f, 7.0f, -5.0f); //Esta es la posici�n inicial del objeto en la escena
	chang.Direccion.x=(float)cos(player1.AngDir*PI/180.0f); //Direcci�n inicial definida por el �ngulo inicial AngDir (x=cos(AngDir), y=0.0, z=sen(AngDir))
	chang.Direccion.y=0.0f;
	chang.Direccion.z=(float)sin(player1.AngDir*PI/180.0f);   
	chang.PosicionCam=CVector(0.0f, player1.CamaraPosAlt, 10.0f); //Posici�n inicial de la c�mara a [DistanciaCam] unidades detr�s del objeto
	chang.ObjetivoCam=player1.PosicionObj;		//La c�mara ve siempre al objeto
...
{
	//Secuencia 1
	KeyFrame1[0].Angt1=	-5.0f;		KeyFrame1[1].Angt1=-5.0f;		KeyFrame1[2].Angt1=-5.0f;
	KeyFrame1[0].Angt2=	0.0f;		KeyFrame1[1].Angt2=0.0f;		KeyFrame1[2].Angt2=0.0f;
	KeyFrame1[0].Angc1=	-25.0f;		KeyFrame1[1].Angc1=-25.0f;		KeyFrame1[2].Angc1=-25.0f;
	KeyFrame1[0].Angc2=	0.0f;		KeyFrame1[1].Angc2=0.0f;		KeyFrame1[2].Angc2=0.0f;
	KeyFrame1[0].Angbi1=-5.0f;		KeyFrame1[1].Angbi1=10.0f;		KeyFrame1[2].Angbi1=-5.0f;
	KeyFrame1[0].Angbi2=0.0f;		KeyFrame1[1].Angbi2=0.0f;		KeyFrame1[2].Angbi2=0.0f;
	KeyFrame1[0].Angbib=-15.0f;		KeyFrame1[1].Angbib=-40.0f;		KeyFrame1[2].Angbib=-15.0f;
	KeyFrame1[0].Angbd1=5.0f;		KeyFrame1[1].Angbd1=-10.0f;		KeyFrame1[2].Angbd1=5.0f;
	KeyFrame1[0].Angbd2=0.0f;		KeyFrame1[1].Angbd2=0.0f;		KeyFrame1[2].Angbd2=0.0f;
	KeyFrame1[0].Angbdb=-40.0f;		KeyFrame1[1].Angbdb=-15.0f;		KeyFrame1[2].Angbdb=-40.0f;
	KeyFrame1[0].Angpizq=20.0f;		KeyFrame1[1].Angpizq=-30.0f;	KeyFrame1[2].Angpizq=20.0f;
	KeyFrame1[0].Angpder=-30.0f;	KeyFrame1[1].Angpder=20.0f;		KeyFrame1[2].Angpder=-20.0f;
	KeyFrame1[0].Angpizqb=15.0f;	KeyFrame1[1].Angpizqb=20.0f;	KeyFrame1[2].Angpizqb=15.0f;
	KeyFrame1[0].Angpderb=10.0f;	KeyFrame1[1].Angpderb=15.0f;	KeyFrame1[2].Angpderb=10.0f;
	KeyFrame1[0].Angpi=	0.0f;		KeyFrame1[1].Angpi=0.0f;		KeyFrame1[2].Angpi=0.0f;
	KeyFrame1[0].Angpd=	0.0f;		KeyFrame1[1].Angpd=0.0f;		KeyFrame1[2].Angpd=0.0f;
	KeyFrame1[0].Xtor=	0.0f;		KeyFrame1[1].Xtor=0.0f;			KeyFrame1[2].Xtor=0.0f;
	KeyFrame1[0].Ytor=	0.0f;		KeyFrame1[1].Ytor=0.0f;			KeyFrame1[2].Ytor=0.0f;
	...
	KeyFrameOtros[ 0 ][ 0 ].Angc1 =	-25.0f;	KeyFrameOtros[ 0 ][ 1 ].Angc1 =	-25.0f;	KeyFrameOtros[ 0 ][ 2 ].Angc1 =	-25.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angc2 =	0.0f;	KeyFrameOtros[ 0 ][ 1 ].Angc2 =	0.0f;	KeyFrameOtros[ 0 ][ 2 ].Angc2 =	0.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angbi1=	-5.0f;	KeyFrameOtros[ 0 ][ 1 ].Angbi1=	-5.0f;	KeyFrameOtros[ 0 ][ 2 ].Angbi1=	-5.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angbi2=	0.0f;	KeyFrameOtros[ 0 ][ 1 ].Angbi2=	0.0f;	KeyFrameOtros[ 0 ][ 2 ].Angbi2=	0.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angbib=	-15.0f;	KeyFrameOtros[ 0 ][ 1 ].Angbib=	-15.0f;	KeyFrameOtros[ 0 ][ 2 ].Angbib=	-15.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angbd1=	5.0f;	KeyFrameOtros[ 0 ][ 1 ].Angbd1=	5.0f;	KeyFrameOtros[ 0 ][ 2 ].Angbd1=	5.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angbd2=	0.0f;	KeyFrameOtros[ 0 ][ 1 ].Angbd2=	0.0f;	KeyFrameOtros[ 0 ][ 2 ].Angbd2=	0.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angbdb=	-40.0f;	KeyFrameOtros[ 0 ][ 1 ].Angbdb=	-40.0f;	KeyFrameOtros[ 0 ][ 2 ].Angbdb=-40.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angpizq=20.0f;	KeyFrameOtros[ 0 ][ 1 ].Angpizq=20.0f;	KeyFrameOtros[ 0 ][ 2 ].Angpizq=20.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angpder=-20.0f;	KeyFrameOtros[ 0 ][ 1 ].Angpder=-20.0f;	KeyFrameOtros[ 0 ][ 2 ].Angpder=-20.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angpizqb=15.0f;	KeyFrameOtros[ 0 ][ 1 ].Angpizqb=15.0f;	KeyFrameOtros[ 0 ][ 2 ].Angpizqb=15.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angpderb=10.0f;	KeyFrameOtros[ 0 ][ 1 ].Angpderb=10.0f;	KeyFrameOtros[ 0 ][ 2 ].Angpderb=10.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angpi =	0.0f;	KeyFrameOtros[ 0 ][ 1 ].Angpi =	0.0f;	KeyFrameOtros[ 0 ][ 2 ].Angpi =	0.0f;
	KeyFrameOtros[ 0 ][ 0 ].Angpd =	0.0f;	KeyFrameOtros[ 0 ][ 1 ].Angpd =	0.0f;	KeyFrameOtros[ 0 ][ 2 ].Angpd =	0.0f;
	KeyFrameOtros[ 0 ][ 0 ].Xtor  =	0.0f;	KeyFrameOtros[ 0 ][ 1 ].Xtor  =	0.0f;	KeyFrameOtros[ 0 ][ 2 ].Xtor  =	0.0f;
	KeyFrameOtros[ 0 ][ 0 ].Ytor  =	0.0f;	KeyFrameOtros[ 0 ][ 1 ].Ytor  =	0.0f;	KeyFrameOtros[ 0 ][ 2 ].Ytor  =	0.0f;
	...
	KeyFrameOtros[ 1 ][ 0 ].Ztor  =	0.0f;	KeyFrameOtros[ 1 ][ 1 ].Ztor  =	0.0f;	KeyFrameOtros[ 1 ][ 2 ].Ztor=0.0f;
...
void ColisionEsferaPlano(int id, int dir, parametros& player )
{
	...
	for(int j=0; j<maxPlanos; j++)
	{
		if(plano[j].tipo == 2)
		{
			...
			{
				//Ahora se comprueba que el punto de prueba se encuentre dentro de los l�mites del plano
...
int InitGL(GLvoid)										// Aqui se configuran los parametros iniciales de OpenGL
{
	...
	*/
	...
	// para la Spline de la trayectoria autom�tica
	ptsXtramo = 20;
	running = 1;
	trayectoria = 1;
	esperar=0;
...
void ActualizaLuz()
{
	...
	//Aqu� se determina a partir de la inversa de la matriz de modelo de vista el espacio de objeto para la luz
	...
	//y signos opuestos para los par�metros
	glPushMatrix();
		glLoadIdentity();
		glRotatef(-player1modelo.Angt1, 1.0f, 0.0f, 0.0f);
		glRotatef(-player1modelo.Angt2, 0.0f, 1.0f, 0.0f);
...
void DibujaEnemigos()
{
	...
	glPopMatrix();
	 ...
	 //Ene1b
	glPushMatrix();
		glTranslatef(enem1b.PosicionObj.x, enem1b.PosicionObj.y, enem1b.PosicionObj.z);
		glRotatef(enem1b.AngObj, 0.0f, 1.0f, 0.0f);
		glScalef(enem1b.escalaX,enem1b.escalaY,enem1b.escalaZ);
		DibujaEnemigo1();
	glPopMatrix();

	//Ene1c
	glPushMatrix();
		glTranslatef(enem1c.PosicionObj.x, enem1c.PosicionObj.y, enem1c.PosicionObj.z);
		glRotatef(enem1c.AngObj, 0.0f, 1.0f, 0.0f);
		glScalef(enem1c.escalaX,enem1c.escalaY,enem1c.escalaZ);
		DibujaEnemigo1();
	glPopMatrix();
	...
	//Ene3e
	glPushMatrix();
		glTranslatef(enem3e.PosicionObj.x, enem3e.PosicionObj.y+2.4f, enem3e.PosicionObj.z);
		glRotatef(enem3e.AngObj,0.0f, 1.0f, 0.0f);
		glScalef(enem3e.escalaX,enem3e.escalaY,enem3e.escalaZ);
		DibujaEnemigo3a();
	glPopMatrix();
	...
	glPushMatrix();
		...
		glRotatef(enem3f.AngObj, 0.0f, 1.0f, 0.0f);
		glScalef(enem3f.escalaX,enem3b.escalaY,enem3b.escalaZ);
		DibujaEnemigo3b();
	glPopMatrix();

     //Ene3g
	glPushMatrix();
		glTranslatef(enem3g.PosicionObj.x, enem3g.PosicionObj.y+2.4f, enem3g.PosicionObj.z);
		glRotatef(enem3g.AngObj,0.0f, 1.0f, 0.0f);
		glScalef(enem3g.escalaX,enem3g.escalaY,enem3g.escalaZ);
		DibujaEnemigo3a();
	glPopMatrix();

	//Ene3h
	glPushMatrix();
		glTranslatef(enem3h.PosicionObj.x, enem3h.PosicionObj.y+2.4f, enem3h.PosicionObj.z);
		glRotatef(enem3h.AngObj, 0.0f, 1.0f, 0.0f);
		glScalef(enem3h.escalaX,enem3h.escalaY,enem3h.escalaZ);
		DibujaEnemigo3b();
	glPopMatrix();
...
int RenderizaEscena(GLvoid)								// Aqui se dibuja todo lo que aparecera en la ventana
{
	...
	*/
	...
	//Se actualizan los par�metros de iluminaci�n
	glLightfv(GL_LIGHT0, GL_POSITION, LightPos);		// Posicion de la luz1
	glLightfv(GL_LIGHT0, GL_AMBIENT,  LightAmb);		// Componente ambiente
	glLightfv(GL_LIGHT0, GL_DIFFUSE,  LightDif);		// Componente difusa
	glLightfv(GL_LIGHT0, GL_SPECULAR, LightSpc);		// Componente especular
	...
	// Se dibuja el modelo con la iluminaci�n desactivada
	...
	// Se desactiva la m�scara de color para renderizar la escena en negro
	...
	//Se inicializa el buffer stencil con una m�scara de ceros.
	...
	// Finalmente se dibuja todo lo que est� en las �reas iluminadas de la escena. Para eso ahora
	...
	// Se desactiva la prueba de profundidad y del buffer stencil ya que no se utilizar�n mas.
	...
	if(displayVolume == true)
	{
		...
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);     //Volvemos al modo s�lido de nuevo
	}
